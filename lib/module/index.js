...
if (Platform.OS === 'android') {
  SafX = NativeModules.SafX ? NativeModules.SafX : new Proxy({}, {
    get() {
 ...
 */
export function createDocument(data, options) {
  if (!options) options = {};
  const {
    encoding,
    initialName,
 ...
 */
export function writeFile(uriString, data, options) {
  if (!options) options = {};
  const {
    encoding,
    append,
 ...
 */
export function createFile(uriString, options) {
  if (!options) options = {};
  const {
    mimeType
  } = options;
 ...
 */
export function unlink(uriString) {
 ...
 * Rejects if it fails.
 */
export function mkdir(uriString) {
 ...
 */
export function rename(uriString, newName) {
 ...
 */
export function copyFile(srcUri, destUri, options) {
  if (!options) options = {};
  const {
 ...
 * Does not support moving directories.
 */
export function moveFile(srcUri, destUri, options) {
  if (!options) options = {};
  const {
    replaceIfDestinationExists = false
  } = options;
