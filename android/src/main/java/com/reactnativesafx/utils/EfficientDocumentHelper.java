...
public class EfficientDocumentHelper {
  ...
  public void transferFile(
    ...
    public void unzip(String srcUri, String destUri, Boolean replaceIfExists, Promise promise) {
        Async.execute(new Async.Task<Object>() {
            @Override
            public Object doAsync() {
                try {
                    ...
                    return performUnzip(srcUriU, destUriU, replaceIfExists, promise);
                } catch (Exception e) {
                    return e;
                }
            }
    ...
    private boolean performUnzip(Uri srcUri, Uri odestUri, Boolean replaceIfExists, Promise promise) {
        try {
            ...
            while ((zipEntry = zis.getNextEntry()) != null) {
                ...
                for(int i = 0; i <= parts.length - 1; i++){
                    ...
                    if(i == parts.length - 1){
                        ...
                        try {
                           isExists = getDocumentUri(destUri.toString()+"%2F"+parts[i], false, true) != null;
                        ...
                        if(zipEntry.isDirectory()){
                            ...
                            if(!isExists){
                                ...
                                destUri = getDocumentUri(destUri.toString() + "%2F" + parts[i], false, true);
                            }
                            //write to file
                        ...
                        if(getStat(destUri) != null) {
                            destUri = Uri.parse(destUri.toString() + "%2F" + parts[i]);
                            continue;
                        }
                        destUri = createDirectory(destUri, parts[i]);
                    }
